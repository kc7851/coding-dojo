function solution1(arr, divisor) {
  ...
  return numberAscendingSort(answer);
}

function solution2(arr, divisor) {
  const answer = arr.filter(value => value % divisor === 0);
  return answer.length > 0 ? numberAscendingSort(answer) : [-1];
}

function numberAscendingSort(arr) {
...
test("sorting", () => {
  expect(numberAscendingSort([2, 36, 1, 3])).toEqual([1, 2, 3, 36]);
  expect(numberAscendingSort([5, 4, 3, 2, 1])).toEqual([1, 2, 3, 4, 5]);
});
